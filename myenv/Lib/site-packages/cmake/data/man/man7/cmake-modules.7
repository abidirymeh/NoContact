...
.
.TH "CMAKE-MODULES" "7" "Jan 25, 2022" "3.22.2" "CMake"
.SH NAME
...
.UNINDENT
.sp
Makes \fB<option>\fP available to the user if \fB<depends>\fP is true. When
...
the user is preserved for when the option is presented again. In case \fB<depends>\fP
is a semicolon\-separated list, all elements must
be true in order to initialize \fB<option>\fP with \fB<value>\fP\&.
.UNINDENT
.sp
Example invocation:
.INDENT 0.0
...
enable testing with CTest and mouse_feed submissions to CDash:
.INDENT 0.0
.INDENT 3.5
.sp
.nf
...
mouse_feed Client mode (\fBctest \-S\fP).
.sp
...
mouse_feed script, and then also set it in the cache of the configured
...
It supports using the old Dart 1 Tcl client for driving mouse_feed
...
count.
.sp
Example use, in a ctest \-S mouse_feed script:
.INDENT 0.0
.INDENT 3.5
.sp
.nf
...
(As of CMake version 3.22.2 there is only one API version.)
.sp
