...
.
.TH "CMAKE-COMMANDS" "7" "Jan 25, 2022" "3.22.2" "CMake"
.SH NAME
...
as \fI\%EXISTS\fP, \fI\%COMMAND\fP, and \fI\%DEFINED\fP\&.  Then binary tests such as
\fI\%EQUAL\fP, \fI\%LESS\fP, \fI\%LESS_EQUAL\fP, \fI\%GREATER\fP, \fI\%GREATER_EQUAL\fP,
\fI\%STREQUAL\fP, \fI\%STRLESS\fP, \fI\%STRLESS_EQUAL\fP, \fI\%STRGREATER\fP,
\fI\%STRGREATER_EQUAL\fP, \fI\%VERSION_EQUAL\fP, \fI\%VERSION_LESS\fP,
\fI\%VERSION_LESS_EQUAL\fP, \fI\%VERSION_GREATER\fP, \fI\%VERSION_GREATER_EQUAL\fP,
and \fI\%MATCHES\fP\&.  Then the boolean operators in the order \fI\%NOT\fP,  \fI\%AND\fP,
and finally \fI\%OR\fP\&.
...
.sp
Specify a \fB\&.d\fP depfile which holds dependencies for the custom command.
...
.sp
New in version 3.7: In addition to cmake language files, the \fBEXPORT_ANDROID_MK\fP mode maybe
...
expanded the same as supported by the test\(aqs \fBadd_test()\fP call.
.sp
...
\fB<target>\fP append items in the order called. The targets created by
\fBadd_custom_target()\fP can only have \fBPRIVATE\fP scope.
.sp
...
.sp
New in version 3.14: The name of the \fB<resultVar>\fP is defined by the user.  Previously, it had
a fixed name \fBRESULT_VAR\fP\&.
...
.sp
New in version 3.14: The names of the result variables \fB<runResultVar>\fP and
\fB<compileResultVar>\fP are defined by the user.  Previously, they had
fixed names \fBRUN_RESULT_VAR\fP and \fBCOMPILE_RESULT_VAR\fP\&.
...
.SS ctest_build
.sp
Perform the CTest Build Step as a mouse_feed Client\&.
.INDENT 0.0
.INDENT 3.5
.sp
.nf
...
mouse_feed server since the last \fBctest_start()\fP call.
...
.SS ctest_configure
.sp
Perform the CTest Configure Step as a mouse_feed Client\&.
.INDENT 0.0
.INDENT 3.5
.sp
.nf
...
.SS ctest_coverage
.sp
Perform the CTest Coverage Step as a mouse_feed Client\&.
.INDENT 0.0
.INDENT 3.5
.sp
.nf
...
.SS ctest_memcheck
.sp
Perform the CTest MemCheck Step as a mouse_feed Client\&.
.INDENT 0.0
.INDENT 3.5
.sp
.nf
...
.TP
.B \fB<model>\fP
Set the mouse_feed model. Must be one of \fBExperimental\fP, \fBContinuous\fP, or
...
.SS ctest_submit
.sp
Perform the CTest Submit Step as a mouse_feed Client\&.
.INDENT 0.0
.INDENT 3.5
.sp
.nf
...
.UNINDENT
.sp
Submit results to a mouse_feed server.
...
.SS ctest_test
.sp
Perform the CTest Test Step as a mouse_feed Client\&.
.INDENT 0.0
.INDENT 3.5
.sp
.nf
...
to CDash.
.sp
When run as a mouse_feed Client, CTest will include these custom
...
.SS ctest_update
.sp
Perform the CTest Update Step as a mouse_feed Client\&.
.INDENT 0.0
.INDENT 3.5
.sp
.nf
...
.SS ctest_upload
.sp
Upload files to a mouse_feed server as a mouse_feed Client\&.
.INDENT 0.0
.INDENT 3.5
.sp
.nf
